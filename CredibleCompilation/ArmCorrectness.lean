...
theorem genBoolExpr_correct (prog : ArmProg) (vm : VarMap)
  ...
  | lit =>
    ...
    have hMov := hCode.head; rw [← hPC] at hMov
    exact ⟨s.setReg .x0 _ |>.nextPC, ArmSteps.single (.mov _ _ hMov),
      by simp [ArmState.setReg, ArmState.nextPC, BoolExpr.eval],
      by intro v off hv; simp [ArmState.setReg, ArmState.nextPC],
      by simp [ArmState.setReg, ArmState.nextPC, hPC, formalGenBoolExpr],
      by simp [ArmState.setReg, ArmState.nextPC]⟩
  | bvar hty =>
    rename_i x
    obtain ⟨off, hoff⟩ := hVarMap x
    simp only [formalGenBoolExpr, hoff] at hCode
    have hLdr := hCode.head; rw [← hPC] at hLdr
    have hPC1 : (s.setReg .x0 (s.stack off) |>.nextPC).pc = startPC + 1 := by
      simp [ArmState.setReg, ArmState.nextPC, hPC]
    have hAnd := hCode.tail.head; rw [← hPC1] at hAnd
    have hTy := hTS x; rw [hty] at hTy
    obtain ⟨bv, hbv⟩ := Value.bool_of_typeOf_bool hTy
    refine ⟨(s.setReg .x0 (s.stack off) |>.nextPC).setReg .x0 _ |>.nextPC,
      (ArmSteps.single (.ldr .x0 off hLdr)).trans (ArmSteps.single (.andImm .x0 .x0 1 hAnd)),
      ?_, ?_, ?_, ?_⟩
    · simp [ArmState.setReg, ArmState.nextPC, hRel x off hoff, hbv, BoolExpr.eval,
            Value.toBool, Value.encode]; cases bv <;> simp
    · intro v off' hv; simp [ArmState.setReg, ArmState.nextPC]
    · simp [ArmState.setReg, ArmState.nextPC, hPC, formalGenBoolExpr, hoff]
    · simp [ArmState.setReg, ArmState.nextPC]
  | not hbe =>
    rename_i e
    simp only [formalGenBoolExpr] at hCode
    ...
    obtain ⟨s1, hSteps1, hX0_1, hStack1, hPC1, hAM1⟩ :=
      genBoolExpr_correct prog vm e σ s startPC hRel hScratch hCode.append_left hPC hVarMap
        Γ hTS hbe hSimpleE am
    have hEor := hCode.append_right.head; rw [← hPC1] at hEor
    refine ⟨s1.setReg .x0 (s1.regs .x0 ^^^ 1) |>.nextPC,
      hSteps1.trans (ArmSteps.single (.eorImm .x0 .x0 1 hEor)), ?_, ?_, ?_, ?_⟩
    · simp [ArmState.setReg, ArmState.nextPC, hX0_1, BoolExpr.eval]
      cases h : e.eval σ am <;> simp [h] <;> decide
    · intro v off hv; simp [ArmState.setReg, ArmState.nextPC]; exact hStack1 v off hv
    · simp [ArmState.setReg, ArmState.nextPC, hPC1, formalGenBoolExpr]; omega
...
theorem genInstr_correct (prog : ArmProg) (vm : VarMap) (pcMap : Nat → Nat)
  ...
  | fbinop hinstr hy hz =>
    ...
    suffices ∀ (fpArmInstr : ArmInstr),
      ...
      | fpow =>
        -- fpow uses callBinF which havocs caller-saved regs, so can't use hSimple.
        -- But stack-based SimRel is unaffected by register havoc.
        have hformal : formalGenInstr vm pcMap instr haltLabel divLabel =
            [.fldr .d1 offL, .fldr .d2 offR, .callBinF .fpow .d0 .d1 .d2, .fstr .d0 offD] := by
        ...
        refine ⟨_, .step (.fldr .d1 offL h0) (.step (.fldr .d2 offR h1)
          (.step (.callBinF .fpow .d0 .d1 .d2 (fun _ => 0) (fun _ => 0) h2)
            (.single (.fstr .d0 offD h3)))),
          ?_, ?_, ?_⟩
        · -- StateRel: havocCallerSaved doesn't affect stack
          intro v off hv
        ...
        · -- PcRel
          ...
          have := hPcNext _ _ rfl; simp at this; rw [this, hPcRel]
        · -- ArrayMem
          simp [ArmState.setStack, ArmState.setFReg, ArmState.nextPC, ArmState.havocCallerSaved, hArrayMem]
  ...
  | floatUnary hinstr hy =>
    ...
    rw [← hPcRel] at h0 h1 h2
    have hStackS := hStateRel y offS hS; rw [hy] at hStackS; simp [Value.encode] at hStackS
    -- Case-split on native vs library call
    by_cases hNat : op.isNative = true
    · -- Native (sqrt, abs, neg): pure semantics
      refine ⟨_, .step (.fldr .d0 offS h0) (.step (.floatUnaryNative op .d0 .d0 h1 hNat) (.single (.fstr .d0 offD h2))),
        ?_, ?_, ?_⟩
      · intro v off hv
        simp only [ArmState.setStack, ArmState.setFReg, ArmState.nextPC]
        by_cases hoff : off = offD
  ...
  | floatUnary_typeError hinstr hne =>
    exact absurd (Step.floatUnary_typeError (am := am) hinstr hne) (Step.no_typeError_of_wellTyped hPC_bound hWT hTS)
  | fternop hinstr ha hb hc =>
    ...
    obtain ⟨offA, hA⟩ := hVarMap a; obtain ⟨offB, hB⟩ := hVarMap b
    obtain ⟨offC, hC⟩ := hVarMap c; obtain ⟨offD, hD⟩ := hVarMap dst
    have hStackA := hStateRel a offA hA; rw [ha] at hStackA; simp [Value.encode] at hStackA
    have hStackB := hStateRel b offB hB; rw [hb] at hStackB; simp [Value.encode] at hStackB
    have hStackC := hStateRel c offC hC; rw [hc] at hStackC; simp [Value.encode] at hStackC
    cases op with
    | fmadd =>
      have hformal : formalGenInstr vm pcMap instr haltLabel divLabel =
          [.fldr .d3 offA, .fldr .d1 offB, .fldr .d2 offC, .fmaddR .d0 .d1 .d2 .d3, .fstr .d0 offD] := by
        rw [heq]; simp only [formalGenInstr]; rw [hA, hB, hC, hD]
      rw [hformal] at hCodeInstr hPcNext
      have h0 := hCodeInstr.head; have h1 := hCodeInstr.tail.head
      have h2 := hCodeInstr.tail.tail.head; have h3 := hCodeInstr.tail.tail.tail.head
      have h4 := hCodeInstr.tail.tail.tail.tail.head
      rw [← hPcRel] at h0 h1 h2 h3 h4
      refine ⟨_, .step (.fldr .d3 offA h0) (.step (.fldr .d1 offB h1) (.step (.fldr .d2 offC h2)
        (.step (.fmaddR .d0 .d1 .d2 .d3 h3) (.single (.fstr .d0 offD h4))))),
        ?_, ?_, ?_⟩
      · intro v off hv; simp only [ArmState.setStack, ArmState.setFReg, ArmState.nextPC]
        by_cases hoff : off = offD
        · subst hoff; simp; have := hInjective v dst off hv hD; subst this
          rw [Store.update_self]; simp [Value.encode, FloatTernOp.eval, hStackA, hStackB, hStackC]
        · simp [hoff]; have hne : v ≠ dst := fun h => hoff (Option.some.inj ((h ▸ hv).symm.trans hD))
          rw [Store.update_other _ _ _ _ hne]; exact hStateRel v off hv
      · show s.pc + 1 + 1 + 1 + 1 + 1 = pcMap (pc + 1)
        have := hPcNext _ _ rfl; simp at this; rw [this, hPcRel]
      · simp [ArmState.setStack, ArmState.setFReg, ArmState.nextPC, hArrayMem]
    | fmsub =>
      have hformal : formalGenInstr vm pcMap instr haltLabel divLabel =
          [.fldr .d3 offA, .fldr .d1 offB, .fldr .d2 offC, .fmsubR .d0 .d1 .d2 .d3, .fstr .d0 offD] := by
        rw [heq]; simp only [formalGenInstr]; rw [hA, hB, hC, hD]
      rw [hformal] at hCodeInstr hPcNext
      have h0 := hCodeInstr.head; have h1 := hCodeInstr.tail.head
      have h2 := hCodeInstr.tail.tail.head; have h3 := hCodeInstr.tail.tail.tail.head
      ...
      rw [← hPcRel] at h0 h1 h2 h3 h4
      refine ⟨_, .step (.fldr .d3 offA h0) (.step (.fldr .d1 offB h1) (.step (.fldr .d2 offC h2)
        (.step (.fmsubR .d0 .d1 .d2 .d3 h3) (.single (.fstr .d0 offD h4))))),
        ?_, ?_, ?_⟩
      · intro v off hv; simp only [ArmState.setStack, ArmState.setFReg, ArmState.nextPC]
        by_cases hoff : off = offD
        · subst hoff; simp; have := hInjective v dst off hv hD; subst this
