var i : int, j : int, k : int, rep : int, qa : float, fw : float,
    fuzz : float, buzz : float, fizz : float;
array za[708] : float, zb[708] : float, zr[708] : float,
...
while (rep <= 8600000) {
  ...
  while (j <= 6) {
    ...
    while (k <= 100) {
      ...
      za[(j - 1) * 101 + k] := za[(j - 1) * 101 + k] + fw * (qa - za[(j - 1) * 101 + k]);
      k := k + 1
    };
    j := j + 1
  };
