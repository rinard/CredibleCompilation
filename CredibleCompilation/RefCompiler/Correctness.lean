...
private theorem compileExprs_correct (args : List SExpr) (offset nextTmp : Nat)
    ...
    ∃ σ', FragExec p offset σ_tac (offset + (compileExprs args offset nextTmp).1.length) σ' am am ∧
      (∀ v, v.isTmp = false → v.isFTmp = false → σ' v = σ_tac v) ∧
      (∀ k, k < nextTmp → σ' (tmpName k) = σ_tac (tmpName k)) ∧
      (∀ k, k < nextTmp → σ' (ftmpName k) = σ_tac (ftmpName k)) := by
  induction args generalizing offset nextTmp σ_tac with
  | nil =>
    simp only [compileExprs, List.length_nil, Nat.add_zero]
    exact ⟨σ_tac, FragExec.rfl' _ _ _ _, fun _ _ _ => rfl, fun _ _ => rfl, fun _ _ => rfl⟩
  | cons e rest ih =>
    ...
    obtain ⟨σ_e, hexec_e, _, hntmp_e, hprev_e, hfprev_e⟩ :=
    ...
    have hagree_e : ∀ v, v.isTmp = false → v.isFTmp = false → σ_e v = σ v := by
    ...
    obtain ⟨σ_rest, hexec_rest, hntmp_rest, hprev_rest, hfprev_rest⟩ :=
      ih (offset + codeE.length) tmp1 σ_e htf_rest hftf_rest htypedv_rest hsafe_rest
        hagree_e hcodeRest
    rw [hrrest] at hexec_rest; simp at hexec_rest
    refine ⟨σ_rest, ?_, ?_, ?_, ?_⟩
    ...
    · -- Non-temp vars: chain hntmp_rest ∘ hntmp_e
      intro v hv1 hv2
      rw [hntmp_rest v hv1 hv2, hntmp_e v hv1 hv2]
...
theorem compileStmt_correct (s : Stmt) (fuel : Nat) (σ σ' : Store) (am am' : ArrayMem)
  ...
  | print fmt args =>
    ...
    · obtain ⟨rfl, rfl⟩ := Prod.mk.inj (Option.some.inj hinterp)
      ...
      obtain ⟨σ_exprs, hexec_exprs, hntmp, _, _⟩ :=
        compileExprs_correct args offset nextTmp σ σ_tac am p htmpfree hftmpfree htypedv hsafe hagree hcodeExprs
      ...
      refine ⟨σ_exprs, ?_, fun v hv1 hv2 => by rw [hntmp v hv1 hv2]; exact hagree v hv1 hv2⟩
