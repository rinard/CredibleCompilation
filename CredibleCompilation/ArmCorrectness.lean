...
theorem verifiedGenInstr_correct (prog : ArmProg) (layout : VarLayout) (pcMap : Nat → Nat)
  ...
  | binop_divByZero hinstr hy hz hs =>
    ...
    obtain ⟨s1, _, hSteps1N, _, hLV_1, hRel1, _, hPC1, hAM1, hRegs1⟩ :=
    ...
    obtain ⟨s2, _, hSteps2N, _, hRV_2, hRel2, _, hPC2_, hAM2, hRegs2⟩ :=
  ...
  | arrLoad_boundsError hinstr hidx hbounds =>
    ...
    | false =>
      ...
      obtain ⟨s1, _, hSteps1N, _, hIdx_1, _hRel1, _, hPC1, _, _⟩ :=
  ...
  | printInt hinstr =>
    ...
    obtain ⟨s1, _, hSteps1N, _, _, hRel1, _, hPC1, hAM1, _⟩ :=
  ...
  | printFloat hinstr =>
    ...
    obtain ⟨s1, _, hSteps1N, _, _, hRel1, _, hPC1, hAM1, _, _⟩ :=
    ...
    | int n =>
      ...
      | some (.stack off) =>
        have hInstrs : instrs = formalLoadImm64 .x0 n ++ vStoreVar layout x .x0 := by
          simp [verifiedGenInstr, hRC, hII, hLocX] at hSome; exact hSome.symm
        rw [hInstrs] at hCodeInstr hPcNext
        have hCodeL := hCodeInstr.append_left
        have hCodeR := hCodeInstr.append_right
        obtain ⟨s1, _, hSteps1N, _, hFregs1, hx0, hStack1, hPC1, hRegs1, hAM1⟩ :=
      ...
      | some (.ireg r) =>
        ...
        have hne : (r == ArmReg.x0) = false := by
          cases hr : r == ArmReg.x0
          · rfl
          · simp [beq_iff_eq] at hr; exact absurd (hr ▸ hLocX) (hRegConv.not_x0 x)
        ...
        refine ⟨s1.setReg r (s1.regs .x0) |>.nextPC,
          ...
          ⟨by rw [hx0]; exact (ExtStateRel.update_ireg hRel1 hInjective hLocX).nextPC,
           by show s1.pc + 1 = pcMap (pc + 1)
              ...
              rw [this, hPC1]; omega,
           by simp [hAM1, hArrayMem]⟩⟩
      | some (.freg _) => simp [verifiedGenInstr, hRC, hII, hLocX] at hSome
      | none => simp [verifiedGenInstr, hRC, hII, hLocX] at hSome
    | bool b =>
      match hLocX : layout x with
      | some (.stack off) =>
    ...
    | float f =>
      ...
      | some (.freg r) =>
        ...
        have hInstrs : instrs = formalLoadImm64 .x0 f ++ [ArmInstr.fmovToFP r .x0] := by
          simp [verifiedGenInstr, hRC, hII, hLocX, vStoreVarFP] at hSome ⊢
          exact hSome.symm
        rw [hInstrs] at hCodeInstr hPcNext
        have hCodeL := hCodeInstr.append_left
        have hCodeR := hCodeInstr.append_right
        ...
        · show s2.pc = pcMap (pc + 1)
          ...
          rw [hPC2]; omega
        · simp [hAM2, hAM1, hArrayMem]
      | some (.ireg _) => simp [verifiedGenInstr, hRC, hII, hLocX] at hSome
      | none => simp [verifiedGenInstr, hRC, hII, hLocX] at hSome
  | copy hinstr =>
    rename_i x y
    ...
    · -- Int copy path: y not in freg
      ...
      · -- non-freg → freg copy: code = vLoadVar y x0 ++ [fmovToFP rf x0]
        ...
        have hCodeL := hCodeInstr.append_left
        have hCodeR := hCodeInstr.append_right
        obtain ⟨s1, _, hSteps1N, _, hX0_1, hRel1, hFregs1, hPC1, hAM1, _⟩ :=
      ...
      · have hNotFregX : ∀ r, layout x ≠ some (.freg r) := fun r h => hXFreg ⟨r, h⟩
        ...
        obtain ⟨s1, _, hSteps1N, _, hX0_1, hRel1, hFregs1, hPC1, hAM1, _⟩ :=
          vLoadVar_exec prog layout y .x0 σ s (pcMap pc) hStateRel hRegConv hPcRel hCodeL
            (Or.inl rfl) hNotFregY (hMapped y (by simp [heq, TAC.vars]))
        have hSteps1 := ArmStepsN_to_ArmSteps hSteps1N
        obtain ⟨s2, _, hSteps2N, _, hRel2, hPC2, hAM2⟩ :=
    ...
    suffices hSimple : ∀ (armOp : ArmInstr),
      ...
      | div =>
        ...
        obtain ⟨s2, _, hSteps2N, _, hRV_2, hRel2, _, hPC2_, hAM2, hRegs2⟩ :=
          vLoadVar_eff_exec prog layout z σ s1
            (pcMap pc + (vLoadVar layout y lv_reg).length) .x2
            hRel1 hRegConv hPC1 hNotFregZ (Or.inr (Or.inr rfl)) (hMapped z (by simp [heq, TAC.vars])) hCodeB
        have hSteps2 := ArmStepsN_to_ArmSteps hSteps2N
        have hPC2 : s2.pc = pcMap pc + (vLoadVar layout y lv_reg).length +
            (vLoadVar layout z rv_reg).length := hPC2_
        -- s2.regs lv_reg = a
  ...
  | iftrue hinstr hcond =>
    ...
    | not inner =>
      cases inner with
      | cmp op a b =>
        simp only [verifiedGenInstr, hRC, hII, hSimpleBV] at hSome
        ...
        | var va => cases b with
          | var vb =>
            ...
            obtain ⟨s1, _, hSteps1N, _, hVal1, hRel1, _, hPC1, hAM1, hRegs1⟩ :=
            ...
            have hSteps1 := ArmStepsN_to_ArmSteps hSteps1N
            obtain ⟨s2, _, hSteps2N, _, hVal2, hRel2, _, hPC2, hAM2, hRegs2⟩ :=
        ...
        | lit na => cases b with
          | var vb =>
            ...
            have hCodeB := hCodeInstr.append_left.append_right
            have hCodeCmpBCond := hCodeInstr.append_right
            obtain ⟨s1, _, hSteps1N, _, hFregs1, hX1, hStack1, hPC1, hRegs1, hAM1⟩ :=
      ...
      | fcmp fop a b =>
        ...
        | var va => cases b with
          | var vb =>
            ...
            let a_freg := match layout va with | some (.freg r) => r | _ => ArmFReg.d1
            let b_freg := match layout vb with | some (.freg r) => r | _ => ArmFReg.d2
            simp only [] at hCodeInstr
            have hCodeA := hCodeInstr.append_left.append_left
            have hCodeB := hCodeInstr.append_left.append_right
            have hCodeFcmpBCond := hCodeInstr.append_right
            obtain ⟨s1, _, hSteps1N, _, hVal1, hRel1, _, hPC1, hAM1, hFregs1, _⟩ :=
              vLoadVarFP_eff_exec prog layout va σ s (pcMap pc) .d1 hStateRel hRegConv hPcRel
                hNotIregA (Or.inr (Or.inl rfl))
                (hMapped va (by simp [TAC.vars, BoolExpr.vars, Expr.freeVars])) hCodeA
            have hSteps1 := ArmStepsN_to_ArmSteps hSteps1N
            obtain ⟨s2, _, hSteps2N, _, hVal2, hRel2, _, hPC2, hAM2, hFregs2, _⟩ :=
            ...
            have hPC2' : s2.pc = pcMap pc + (vLoadVarFP layout va a_freg ++ vLoadVarFP layout vb b_freg).length := by
              rw [List.length_append, ← Nat.add_assoc]; exact hPC2
            have hFcmpI := hPC2'.symm ▸ hCodeFcmpBCond.head
            let s3 := { s2 with flags := Flags.mk (s2.fregs a_freg) (s2.fregs b_freg), pc := s2.pc + 1 }
            have hPC3 : s3.pc = pcMap pc + (vLoadVarFP layout va a_freg ++ vLoadVarFP layout vb b_freg).length + 1 := by
              simp [s3, hPC2']
            have hBCond := hPC3.symm ▸ hCodeFcmpBCond.tail.head
            ...
            have hCondFalse : s3.flags.condHolds
              ...
              show ({ lhs := s2.fregs a_freg, rhs := s2.fregs b_freg } : Flags).condHolds _ = false
              rw [hA_s2, hB_s2]
              cases fop <;> exact (Flags.condHolds_float_correct _ fA fB).trans hfcmp_false
            refine ⟨{ s3 with pc := pcMap l_var },
              (hSteps1.trans hSteps2).trans (.step (.fcmpRR a_freg b_freg hFcmpI) (.single (.bCond_taken _ _ hBCond ?_))),
              ⟨fun v loc hv => hRel2 v loc hv, rfl, by simp [s3, hAM2, hAM1, hArrayMem]⟩⟩
          ...
          | flit fb =>
            ...
            obtain ⟨s1, _, hSteps1N, _, hVal1, hRel1, _, hPC1, hAM1, hFregs1, _⟩ :=
              vLoadVarFP_eff_exec prog layout va σ s (pcMap pc) .d1 hStateRel hRegConv hPcRel
                ...
                (hMapped va (by simp [TAC.vars, BoolExpr.vars, Expr.freeVars])) hCodeA
            have hSteps1 := ArmStepsN_to_ArmSteps hSteps1N
            obtain ⟨s2, _, hSteps2N, _, hD2val, hRel2, hPC2, hAM2, hFregs2⟩ :=
              loadFloatExpr_exec prog layout (.flit fb) .d2 σ am s1 _ hRel1 hRegConv hPC1
                (Or.inr ⟨fb, rfl⟩) tyCtx hTS hb_ty hWTL
                (fun v hv => by simp [Expr.freeVars] at hv)
                (Or.inr (Or.inr rfl)) hCodeFlit
            have hSteps2 := ArmStepsN_to_ArmSteps hSteps2N
            have hA_s2 : s2.fregs a_freg = fA := by
              have := eff_freg_val_preserved layout va σ s2 .d1 .d2 hRel2 hNotIregA
                hFregs2 hVal1 (by decide)
              rw [this, hfA]; rfl
            have hD2_s2 : s2.fregs .d2 = fb := by simp [hD2val, Expr.eval, Value.toFloat]
            have hPC2' : s2.pc = pcMap pc + (vLoadVarFP layout va a_freg ++ (formalLoadImm64 .x0 fb ++ [ArmInstr.fmovToFP .d2 .x0])).length := by
              rw [List.length_append, ← Nat.add_assoc]; exact hPC2
            have hFcmpI := hPC2'.symm ▸ hCodeFcmpBCond.head
            let s3 := { s2 with flags := Flags.mk (s2.fregs a_freg) (s2.fregs .d2), pc := s2.pc + 1 }
            have hPC3 : s3.pc = pcMap pc + (vLoadVarFP layout va a_freg ++ (formalLoadImm64 .x0 fb ++ [ArmInstr.fmovToFP .d2 .x0])).length + 1 := by
              simp [s3, hPC2']
            have hBCond := hPC3.symm ▸ hCodeFcmpBCond.tail.head
            simp only [BoolExpr.eval, Expr.eval, Value.toFloat, hfA] at hfcmp_false
            have hCondFalse : s3.flags.condHolds
                (match fop with | .feq => Cond.eq | .fne => .ne | .flt => .lt | .fle => .le) = false := by
              show ({ lhs := s2.fregs a_freg, rhs := s2.fregs .d2 } : Flags).condHolds _ = false
              rw [hA_s2, hD2_s2]
              cases fop <;> exact (Flags.condHolds_float_correct _ fA fb).trans hfcmp_false
            refine ⟨{ s3 with pc := pcMap l_var },
              (hSteps1.trans hSteps2).trans (.step (.fcmpRR a_freg .d2 hFcmpI) (.single (.bCond_taken _ _ hBCond ?_))),
              ⟨fun v loc hv => hRel2 v loc hv, rfl, by simp [s3, hAM2, hAM1, hArrayMem]⟩⟩
            show s3.flags.condHolds _ = true
            cases fop <;> (rw [Cond.negate_correct]; simp [hCondFalse])
          | _ => rcases hSimpleB with ⟨_, h⟩ | ⟨_, h⟩ <;> simp at h
        | flit fa =>
          ...
          | var vb =>
            ...
            let b_freg := match layout vb with | some (.freg r) => r | _ => ArmFReg.d2
            simp only [] at hCodeInstr
            -- Codegen swaps: loadB ++ loadA for flit/var
            ...
            obtain ⟨s1, _, hSteps1N, _, hVal1, hRel1, _, hPC1, hAM1, hFregs1, _⟩ :=
              vLoadVarFP_eff_exec prog layout vb σ s (pcMap pc) .d2 hStateRel hRegConv hPcRel
                ...
                (hMapped vb (by simp [TAC.vars, BoolExpr.vars, Expr.freeVars])) hCodeB
            have hSteps1 := ArmStepsN_to_ArmSteps hSteps1N
            obtain ⟨s2, _, hSteps2N, _, hD1val, hRel2, hPC2, hAM2, hFregs2⟩ :=
              loadFloatExpr_exec prog layout (.flit fa) .d1 σ am s1 _ hRel1 hRegConv hPC1
                (Or.inr ⟨fa, rfl⟩) tyCtx hTS ha_ty hWTL
                (fun v hv => by simp [Expr.freeVars] at hv)
                (Or.inr (Or.inl rfl)) hCodeFlit
            have hSteps2 := ArmStepsN_to_ArmSteps hSteps2N
            have hB_s2 : s2.fregs b_freg = fB := by
              have := eff_freg_val_preserved layout vb σ s2 .d2 .d1 hRel2 hNotIregB
                hFregs2 hVal1 (by decide)
              rw [this, hfB]; rfl
            have hD1_s2 : s2.fregs .d1 = fa := by simp [hD1val, Expr.eval, Value.toFloat]
            have hPC2' : s2.pc = pcMap pc + (vLoadVarFP layout vb b_freg ++ (formalLoadImm64 .x0 fa ++ [ArmInstr.fmovToFP .d1 .x0])).length := by
              rw [List.length_append, ← Nat.add_assoc]; exact hPC2
            have hFcmpI := hPC2'.symm ▸ hCodeFcmpBCond.head
            let s3 := { s2 with flags := Flags.mk (s2.fregs .d1) (s2.fregs b_freg), pc := s2.pc + 1 }
            have hPC3 : s3.pc = pcMap pc + (vLoadVarFP layout vb b_freg ++ (formalLoadImm64 .x0 fa ++ [ArmInstr.fmovToFP .d1 .x0])).length + 1 := by
              simp [s3, hPC2']
            have hBCond := hPC3.symm ▸ hCodeFcmpBCond.tail.head
            simp only [BoolExpr.eval, Expr.eval, Value.toFloat, hfB] at hfcmp_false
            have hCondFalse : s3.flags.condHolds
                (match fop with | .feq => Cond.eq | .fne => .ne | .flt => .lt | .fle => .le) = false := by
              show ({ lhs := s2.fregs .d1, rhs := s2.fregs b_freg } : Flags).condHolds _ = false
              rw [hD1_s2, hB_s2]
              cases fop <;> exact (Flags.condHolds_float_correct _ fa fB).trans hfcmp_false
            refine ⟨{ s3 with pc := pcMap l_var },
              (hSteps1.trans hSteps2).trans (.step (.fcmpRR .d1 b_freg hFcmpI) (.single (.bCond_taken _ _ hBCond ?_))),
              ⟨fun v loc hv => hRel2 v loc hv, rfl, by simp [s3, hAM2, hAM1, hArrayMem]⟩⟩
            show s3.flags.condHolds _ = true
            cases fop <;> (rw [Cond.negate_correct]; simp [hCondFalse])
          | flit fb =>
            ...
            obtain ⟨s1, _, hSteps1N, _, hD1val, hRel1, hPC1, hAM1, hFregs1⟩ :=
              loadFloatExpr_exec prog layout (.flit fa) .d1 σ am s (pcMap pc) hStateRel hRegConv hPcRel
                (Or.inr ⟨fa, rfl⟩) tyCtx hTS ha_ty hWTL
                (fun v hv => by simp [Expr.freeVars] at hv)
      ...
      | _ =>
        ...
        obtain ⟨s1, _, hSteps1N, _, hX0_1, hRel1, hPC1, hAM1⟩ :=
          verifiedGenBoolExpr_correct prog layout _ σ s (pcMap pc) hStateRel hRegConv
        ...
        | var va => cases b with
          | var vb =>
            ...
            · show s3.pc + 1 = pcMap (pc + 1)
              have hPcN := hPcNext _ _ rfl
              simp [List.length_append, Nat.add_assoc] at hPcN hPC2
              simp only [s3]; rw [hPcN, hPC2]; rfl
          | lit nb =>
            have hNotFregA : ∀ r, layout va ≠ some (.freg r) := hWTL.int_not_freg ha_ty
            have hTyA := hTS va; rw [ha_ty] at hTyA
            obtain ⟨nA, hnA⟩ := Value.int_of_typeOf_int hTyA
      ...
      | fcmp fop a b =>
        ...
        | var va => cases b with
          | var vb =>
            ...
            have hCondTrue : s3.flags.condHolds
              ...
              show ({ lhs := s2.fregs a_freg, rhs := s2.fregs b_freg } : Flags).condHolds _ = true
              rw [hA_s2, hB_s2]
              cases fop <;> exact (Flags.condHolds_float_correct _ fA fB).trans hfcmp_true
            refine ⟨s3.nextPC,
              (hSteps1.trans hSteps2).trans (.step (.fcmpRR a_freg b_freg hFcmpI) (.single (.bCond_fall _ _ hBCond ?_))),
              ⟨ExtStateRel.nextPC (fun v loc hv => hRel2 v loc hv), ?_,
               by simp [ArmState.nextPC, s3, hAM2, hAM1, hArrayMem]⟩⟩
          ...
          | flit fb =>
            ...
            have hCondTrue : s3.flags.condHolds
                (match fop with | .feq => Cond.eq | .fne => .ne | .flt => .lt | .fle => .le) = true := by
              show ({ lhs := s2.fregs a_freg, rhs := s2.fregs .d2 } : Flags).condHolds _ = true
              rw [hA_s2, hD2_s2]
              cases fop <;> exact (Flags.condHolds_float_correct _ fA fb).trans hfcmp_true
            refine ⟨s3.nextPC,
              (hSteps1.trans hSteps2).trans (.step (.fcmpRR a_freg .d2 hFcmpI) (.single (.bCond_fall _ _ hBCond ?_))),
              ⟨ExtStateRel.nextPC (fun v loc hv => hRel2 v loc hv), ?_,
               by simp [ArmState.nextPC, s3, hAM2, hAM1, hArrayMem]⟩⟩
            · show s3.flags.condHolds _ = false
              cases fop <;> (rw [Cond.negate_correct]; simp [hCondTrue])
        ...
        | flit fa =>
          ...
          | var vb =>
            ...
            have hCondTrue : s3.flags.condHolds
                (match fop with | .feq => Cond.eq | .fne => .ne | .flt => .lt | .fle => .le) = true := by
              show ({ lhs := s2.fregs .d1, rhs := s2.fregs b_freg } : Flags).condHolds _ = true
              rw [hD1_s2, hB_s2]
              cases fop <;> exact (Flags.condHolds_float_correct _ fa fB).trans hfcmp_true
            refine ⟨s3.nextPC,
              (hSteps1.trans hSteps2).trans (.step (.fcmpRR .d1 b_freg hFcmpI) (.single (.bCond_fall _ _ hBCond ?_))),
              ⟨ExtStateRel.nextPC (fun v loc hv => hRel2 v loc hv), ?_,
               by simp [ArmState.nextPC, s3, hAM2, hAM1, hArrayMem]⟩⟩
            · show s3.flags.condHolds _ = false
              cases fop <;> (rw [Cond.negate_correct]; simp [hCondTrue])
  ...
  | arrLoad hinstr hidx hbounds =>
    ...
    have heq : instr = .arrLoad dst arr idx ty := Option.some.inj (hInstr.symm.trans hinstr)
    rw [heq] at hSome
    -- Substep 2: case split on ty, extract instruction list
    cases ty with
    | int =>
      -- Substep 1: derive hNotFreg guards (ty is now concrete)
      have hNotFregIdx : ∀ r, layout idx ≠ some (.freg r) :=
        hWTL.int_not_freg (by have := hTS idx; rw [hidx] at this; exact this.symm)
      let idx_reg := match layout idx with | some (.ireg r) => r | _ => ArmReg.x1
      ...
      have hInstrs : instrs = vLoadVar layout idx idx_reg ++
        ...
        simp [verifiedGenInstr, hRC, hII] at hSome; exact hSome.symm
      rw [hInstrs] at hCodeInstr hPcNext
      -- Substep 3: split CodeAt
      have hCodeA := hCodeInstr.append_left
      have hCodeBCD := hCodeInstr.append_right
      -- Substep 4: vLoadVar idx into idx_reg
      obtain ⟨s1, _, hSteps1N, _, hX1_1, hRel1, _, hPC1, hAM1, hRegs1⟩ :=
        vLoadVar_eff_exec prog layout idx σ s (pcMap pc) .x1 hStateRel hRegConv hPcRel
          hNotFregIdx (Or.inr (Or.inl rfl)) (hMapped idx (by simp [heq, TAC.vars])) hCodeA
      have hSteps1 := ArmStepsN_to_ArmSteps hSteps1N
      -- Substep 5: bounds check
      -- s1.regs idx_reg = idxVal
      have hIdxVal : s1.regs idx_reg = idxVal := by rw [hX1_1, hidx]; simp [Value.encode]
      -- case split on boundsSafe
      cases hBS : boundsSafe with
      | true =>
        -- boundsCheck = [], no instructions to execute
        ...
        · obtain ⟨r_dst, hDstLoc⟩ := hDIR
          ...
          refine ⟨s3, hSteps1.trans hStepsArrLd, ⟨hRel4, ?_, hAM3⟩⟩
          · show s3.pc = pcMap (pc + 1)
        ...
        · have hDR : dst_reg = .x0 := by
          ...
          obtain ⟨s4, _, hSteps4N, _, hRel4, hPC5, hAM4⟩ :=
          ...
          refine ⟨s4, hSteps1.trans (hStepsArrLd.trans hSteps4), ⟨hRel4, ?_, hAM_final⟩⟩
          · show s4.pc = pcMap (pc + 1)
      ...
      | false =>
        ...
        have hStepsCmp : ArmSteps prog s1
          ...
          .single (.cmpRI idx_reg (arraySizeBv arrayDecls arr) hCmp)
        let s2 := { s1 with flags := Flags.mk (s1.regs idx_reg) (arraySizeBv arrayDecls arr), pc := s1.pc + 1 }
        -- bCond .hs falls through: condHolds .hs = (size ≤ idxVal) = false
        ...
        -- Substep 6: arrLd dst_reg arr idx_reg
        ...
        have hNotFregDst : ∀ r, layout dst ≠ some (.freg r) := by
          ...
          cases hwti with
          | arrLoad hx _ _ => exact hWTL.int_not_freg hx
        -- Substep 7: vStoreVar + final ExtSimRel
        by_cases hDIR : ∃ r, layout dst = some (.ireg r)
        · obtain ⟨r_dst, hDstLoc⟩ := hDIR
          have hDR : dst_reg = r_dst := by
            change (match layout dst with | some (.ireg r) => r | _ => .x0) = r_dst; rw [hDstLoc]
          ...
          refine ⟨s3, ((hSteps1.trans hStepsCmp).trans hStepsBCond).trans hStepsArrLd,
            ⟨hRel4, ?_, ?_⟩⟩
          · show s3.pc = pcMap (pc + 1)
        ...
        · have hDR : dst_reg = .x0 := by
          ...
          refine ⟨s4, ((hSteps1.trans hStepsCmp).trans hStepsBCond).trans (hStepsArrLd.trans hSteps4),
            ⟨hRel4, ?_, hAM_final⟩⟩
          · show s4.pc = pcMap (pc + 1)
            have := hPcNext _ _ rfl; rw [show dst_reg = ArmReg.x0 from hDR] at this
            simp [hBS] at this; omega
    ...
    | float =>
      ...
      have hNotFregIdx : ∀ r, layout idx ≠ some (.freg r) :=
        hWTL.int_not_freg (by have := hTS idx; rw [hidx] at this; exact this.symm)
      -- dst is float-typed, so not in ireg
      ...
      | true =>
        simp [hBS] at hCodeBCD
        -- farrLd + vStoreVarFP via fp_exec_and_store
        ...
        have hSteps' := ArmStepsN_to_ArmSteps hSteps'N
        exact ⟨s', hSteps1.trans hSteps', hSimRel'⟩
      | false =>
        simp [hBS] at hCodeBCD
        -- cmpImm + bCond
        have hCmp := hCodeBCD.head; rw [← hPC1] at hCmp
        ...
        obtain ⟨s', _, hSteps'N, _, hSimRel'⟩ :=
          fp_exec_and_store prog layout pcMap divLabel boundsLabel pc σ am dst dst_freg
            ...
            (by have := hPcNext _ _ rfl; simp [hBS] at this; omega)
        have hSteps' := ArmStepsN_to_ArmSteps hSteps'N
        exact ⟨s', ((hSteps1.trans hStepsCmp).trans hStepsBCond).trans hSteps', hSimRel'⟩
  | arrStore hinstr hidx hval hbounds =>
    ...
    have hNotFregIdx : ∀ r, layout idx ≠ some (.freg r) :=
      hWTL.int_not_freg (by have := hTS idx; rw [hidx] at this; exact this.symm)
    let idx_reg := match layout idx with | some (.ireg r) => r | _ => ArmReg.x1
    -- case split on float vs non-float
    ...
    · -- float arrStore
      ...
      | true =>
        simp [hBS] at hCodeBCD
        -- vLoadVarFP val into val_freg
        have hCodeValSt := hCodeBCD; rw [← hPC1] at hCodeValSt
        have hCodeVal := hCodeValSt.append_left
        obtain ⟨s2, _, hSteps2N, _, hFVal_2, hRel2, hRegs2eq, hPC2, hAM2, hFregs2, hStack2⟩ :=
          vLoadVarFP_eff_exec prog layout val σ s1 s1.pc .d0 hRel1 hRegConv rfl
            hNotIregVal (Or.inl rfl) (hMapped val (by simp [heq, TAC.vars])) hCodeVal
        have hSteps2 := ArmStepsN_to_ArmSteps hSteps2N
        -- idx_reg preserved (integer reg, FP load doesn't touch it)
        ...
        have hStepsFarrSt : ArmSteps prog s2
            (s2.setArrayMem arr (s2.regs idx_reg) (s2.fregs val_freg) |>.nextPC) :=
          .single (.farrSt arr idx_reg val_freg hFarrSt)
        ...
        have hAM3 : s3.arrayMem = am.write arr idxVal (σ val).toBits := by
          ...
          rfl
        exact ⟨s3, (hSteps1.trans hSteps2).trans hStepsFarrSt, ⟨hRel3, hPC3, hAM3⟩⟩
      | false =>
        simp [hBS] at hCodeBCD
        -- cmpImm + bCond
        have hCmp := hCodeBCD.head; rw [← hPC1] at hCmp
        let s2bc := { s1 with flags := Flags.mk (s1.regs idx_reg) (arraySizeBv arrayDecls arr), pc := s1.pc + 1 }
        have hStepsCmp : ArmSteps prog s1 s2bc :=
          .single (.cmpRI idx_reg (arraySizeBv arrayDecls arr) hCmp)
        have hBoundsAD : idxVal < arraySizeBv arrayDecls arr := by rw [hAD]; exact hbounds
        ...
        have hStepsBCond : ArmSteps prog s2bc s2bc.nextPC :=
          .single (.bCond_fall .hs boundsLabel hBCond hCondFalse)
        ...
        obtain ⟨s2, _, hSteps2N, _, hFVal_2, hRel2, hRegs2eq, hPC2, hAM2, hFregs2, hStack2⟩ :=
          vLoadVarFP_eff_exec prog layout val σ s2bc.nextPC s2bc.nextPC.pc .d0
            ...
            (hMapped val (by simp [heq, TAC.vars])) hCodeVal
        have hSteps2 := ArmStepsN_to_ArmSteps hSteps2N
        ...
        have hAM3 : s3.arrayMem = am.write arr idxVal (σ val).toBits := by
          ...
          rfl
        exact ⟨s3, ((hSteps1.trans hStepsCmp).trans hStepsBCond).trans (hSteps2.trans hStepsFarrSt),
          ⟨hRel3, hPC3, hAM3⟩⟩
    · -- non-float arrStore
      ...
      | true =>
        simp [hBS] at hCodeBCD
        -- vLoadVar val into val_reg
        have hCodeVal := hCodeBCD.append_left; rw [← hPC1] at hCodeVal
        obtain ⟨s2, _, hSteps2N, _, hVal_2, hRel2, _, hPC2, hAM2, hRegs2⟩ :=
          vLoadVar_eff_exec prog layout val σ s1 s1.pc .x2 hRel1 hRegConv rfl
            hNotFregVal (Or.inr (Or.inr rfl)) (hMapped val (by simp [heq, TAC.vars])) hCodeVal
        have hSteps2 := ArmStepsN_to_ArmSteps hSteps2N
        -- show idx_reg preserved across vLoadVar val
        ...
        have hStepsArrSt : ArmSteps prog s2
            (s2.setArrayMem arr (s2.regs idx_reg) (s2.regs val_reg) |>.nextPC) :=
          .single (.arrSt arr idx_reg val_reg hArrSt)
        ...
        have hAM3 : s3.arrayMem = am.write arr idxVal (σ val).toBits := by
          ...
          simp only [s3, ArmState.setArrayMem, ArmState.nextPC, ArrayMem.write,
                hIdxVal2, hValEnc, hAM2, hAM1, hArrayMem, Value.encode, Value.toBits]
          rfl
        exact ⟨s3, (hSteps1.trans hSteps2).trans hStepsArrSt, ⟨hRel3, hPC3, hAM3⟩⟩
      | false =>
        simp [hBS] at hCodeBCD
        -- cmpImm + bCond
        have hCmp := hCodeBCD.head; rw [← hPC1] at hCmp
        ...
        have hAM3 : s3.arrayMem = am.write arr idxVal (σ val).toBits := by
          ...
          simp only [s3, ArmState.setArrayMem, ArmState.nextPC, ArrayMem.write,
                hIdxVal2, hValEnc, hAM2', Value.encode, Value.toBits]
          rfl
        exact ⟨s3, ((hSteps1.trans hStepsCmp).trans hStepsBCond).trans (hSteps2.trans hStepsArrSt),
          ⟨hRel3, hPC3, hAM3⟩⟩
  | fbinop hinstr hy hz =>
    ...
    suffices hSimple : ∀ (fpOp : ArmInstr),
      ...
      | fpow =>
        ...
        obtain ⟨s1, _, hSteps1N, _, hLV_1, hRel1, hRegs1, hPC1, hAM1, hFregs1, hStack1⟩ :=
        ...
        obtain ⟨s2, _, hSteps2N, _, hRV_2, hRel2, hRegs2, hPC2_, hAM2, hFregs2, hStack2⟩ :=
  ...
  | intToFloat hinstr hy =>
    ...
    | some (.stack _) =>
      ...
      obtain ⟨s3, _, hSteps3N, _, hRel3, hPC3, hAM3⟩ :=
      ...
      have hSteps3 := ArmStepsN_to_ArmSteps hSteps3N
  ...
  | floatToInt hinstr hy =>
    ...
    | some (.stack _) =>
      ...
      obtain ⟨s1, _, hSteps1N, _, hD0_1, hRel1, hRegs1, hPC1, hAM1, _, _⟩ :=
  ...
  | floatUnary hinstr hy =>
    ...
    obtain ⟨s1, _, hSteps1N, _, hSR_1, hRel1, hRegs1, hPC1, hAM1, hFregs1, hStack1⟩ :=
  ...
  | fternop hinstr ha hb hc =>
    ...
    obtain ⟨s1, _, hSteps1N, _, hA_1, hRel1, hRegs1, hPC1, hAM1, hFregs1, hStack1⟩ :=
    ...
    obtain ⟨s2, _, hSteps2N, _, hB_2, hRel2, hRegs2, hPC2, hAM2, hFregs2, hStack2⟩ :=
    ...
    obtain ⟨s3, _, hSteps3N, _, hC_3, hRel3, hRegs3, hPC3, hAM3, hFregs3, hStack3⟩ :=
    ...
    have hA_3 : s3.fregs a_reg = va := by
      ...
      | some (.stack _) | none =>
        ...
        rw [this]
        -- d0 ≠ b_reg and d0 ≠ c_reg (by regConventionSafe)
    ...
    have hArmStepReal : ArmStep prog s3
      ...
      · -- fmsub
        ...
        rw [hA_3, hB_3, hC_eq] at step; exact step
    obtain ⟨s_fin, _, hSteps_finN, _, hSimRel⟩ := fp_exec_and_store prog layout pcMap divLabel boundsLabel
    ...
    have hSteps_fin := ArmStepsN_to_ArmSteps hSteps_finN
