...
private theorem collectVars_step_subset {acc : List Var} {instr : TAC} {w : Var}
    ...
    | .ifgoto be _     => be.vars.foldl (fun a v => if a.contains v then a else a ++ [v]) acc
    | .halt            => acc)) :
    w ∈ acc ∨ w ∈ instr.vars := by
  cases instr with
  | const x _ =>
    rcases mem_of_mem_addIfNew_imp hw with h | rfl
    · exact Or.inl h
    · exact Or.inr (by simp [TAC.vars])
  | copy x y =>
    rcases mem_of_mem_addIfNew_imp hw with h | rfl
    · rcases mem_of_mem_addIfNew_imp h with h | rfl
      · exact Or.inl h
  ...
  | binop x _ y z =>
    ...
    · exact Or.inr (by simp [TAC.vars])
  | boolop x be =>
    rcases mem_foldl_addIfNew_imp hw with h | h
    · rcases mem_of_mem_addIfNew_imp h with h | rfl
      · exact Or.inl h
      · exact Or.inr (by simp [TAC.vars])
    · exact Or.inr (by simp [TAC.vars, h])
  | arrLoad x _ idx _ =>
    rcases mem_of_mem_addIfNew_imp hw with h | rfl
    · rcases mem_of_mem_addIfNew_imp h with h | rfl
      · exact Or.inl h
  ...
  | print _ vs =>
    ...
    · exact Or.inl h
    · exact Or.inr (by simp [TAC.vars, h])
  | goto _ => exact Or.inl hw
  | ifgoto be _ =>
    rcases mem_foldl_addIfNew_imp hw with h | h
    · exact Or.inl h
    · exact Or.inr (by simp [TAC.vars, h])
  | halt => exact Or.inl hw
...
private theorem collectVars_code_pred {P : Var → Prop}
  ...
  rw [← Array.foldl_toList]
  intro v hv
  -- Induction: at each step, all acc elements satisfy P
  suffices ∀ (l : List TAC) (acc : List Var),
      (∀ w ∈ acc, P w) →
      (∀ instr ∈ l, ∀ w ∈ instr.vars, P w) →
      ∀ w ∈ l.foldl (fun acc instr => match instr with
        | .const x _ => if acc.contains x then acc else acc ++ [x]
        | .copy x y => let a := if acc.contains x then acc else acc ++ [x]
                        if a.contains y then a else a ++ [y]
        | .binop x _ y z => let a := if acc.contains x then acc else acc ++ [x]
...
private theorem compileBool_noArmReg (b : SBool) (offset nextTmp : Nat)
    (hSrc : Program.SBool.noReservedVars b = true) :
    (∀ instr ∈ (compileBool b offset nextTmp).1, ∀ v ∈ instr.vars, varToArmReg v = none) ∧
    (∀ v ∈ (compileBool b offset nextTmp).2.1.vars, varToArmReg v = none) := by
  ...
  | bvar x =>
    ...
    constructor
    · intro _ h; simp at h
    · intro v hv; simp at hv; rw [hv]; exact varToArmReg_none_of_not_dunder x hSrc
  | cmp _ a b =>
    simp only [Program.SBool.noReservedVars, Bool.and_eq_true] at hSrc
    simp only [compileBool]
    have ⟨ha_i, ha_r⟩ := compileExpr_noArmReg a offset nextTmp hSrc.1
  ...
  | barrRead _ idx =>
    simp only [Program.SBool.noReservedVars] at hSrc
    simp only [compileBool]
    have ⟨hi_i, hi_r⟩ := compileExpr_noArmReg idx offset nextTmp hSrc
    constructor
    · intro instr hmem v hv
      simp [List.mem_append] at hmem
      rcases hmem with h | rfl
